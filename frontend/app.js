...
// ===== OKAMŽITÁ NÁPOVEDA (data-tip) — zobrazí sa hneď, bez oneskorenia natívneho title =====
function _tipBox(){let el=document.getElementById("tipbox");if(!el){el=document.createElement("div");el.id="tipbox";document.body.appendChild(el);}return el;}
function _tipShow(t){const el=_tipBox();el.textContent=t.getAttribute("data-tip")||"";const r=t.getBoundingClientRect();el.style.display="block";const w=el.offsetWidth;let left=r.left+r.width/2-w/2;left=Math.max(6,Math.min(left,window.innerWidth-w-6));el.style.left=left+"px";let top=r.top-el.offsetHeight-7;if(top<6)top=r.bottom+7;el.style.top=top+"px";el.style.opacity="1";}
function _tipHide(){const el=document.getElementById("tipbox");if(el){el.style.opacity="0";el.style.display="none";}}
document.addEventListener("mouseover",e=>{const t=e.target.closest&&e.target.closest("[data-tip]");if(t)_tipShow(t);});
document.addEventListener("mouseout",e=>{const t=e.target.closest&&e.target.closest("[data-tip]");if(t)_tipHide();});
...
const BUY_SOURCES=["faktúra","cenník dodávateľa","odhad","predchádzajúci nákup","manuálne"];
// oprávnenia sa načítajú podľa roly z role_permissions (viď onLogin)
const hasPerm=(code)=>ME.role==="admin"||ME.perms.includes(code);
// koľko minút po prvom pohybe možno ešte dopĺňať doklad (mimo admina / oprávnenia doc.edit.unlimited)
const DOC_ADD_WINDOW_MIN=1440; // 24 h — dá sa upraviť podľa potreby
function docAddAllowed(oldestTs){if(ME.role==="admin"||hasPerm("doc.edit.unlimited"))return true;if(!oldestTs)return true;return ((Date.now()-new Date(oldestTs).getTime())/60000)<=DOC_ADD_WINDOW_MIN;}
// odkaz (deep-link) na konkrétnu položku — na preposlanie medzi užívateľmi
function copyLinkBtn(hash){const url=(location.origin+location.pathname+"#"+hash).replace(/'/g,"\\'");return `<button class="btn ghost sm" data-tip="Kopírovať odkaz na túto položku (dá sa preposlať)" onclick="copyText('${url}');this.textContent='✓ Odkaz skopírovaný';setTimeout(()=>this.textContent='🔗 Odkaz',1400)">🔗 Odkaz</button>`;}
...
async function loadData(){
  const [p,w,l,c,b,tg,pt,ad,pa]=await Promise.all([
    ...
    sb.from("warehouses").select("id,name,address,color,code,sort_order").order("sort_order"),
  ...
  // majetok: provozovne, miestnosti, osoby
  const [prem,rooms,pers]=await Promise.all([
    sb.from("asset_premises").select("id,name,address,sort_order").order("sort_order"),
    sb.from("asset_rooms").select("id,premise_id,name,sort_order").order("sort_order"),
    sb.from("asset_persons").select("id,name,email,note,active,sort_order").order("sort_order")
  ]);
  DATA.premises=prem.data||[];DATA.rooms=rooms.data||[];DATA.persons=pers.data||[];
}
function premiseName(id){const x=DATA.premises.find(p=>p.id===id);return x?x.name:"";}
function roomName(id){const x=DATA.rooms.find(r=>r.id===id);return x?x.name:"";}
function personName(id){const x=DATA.persons.find(p=>p.id===id);return x?x.name:"";}
function roomsOfPremise(pid){return DATA.rooms.filter(r=>String(r.premise_id)===String(pid));}
...
function route(){
  ...
  if(t==="lot"&&id){markTab("stock");$("#view").className="";lotRoute(Number(id));navHash("lot/"+id);return;}
...
// deep-link na konkrétny tovar (šaržu): načíta zásoby a otvorí úpravu tovaru
async function lotRoute(id){tab="stock";await loadStock();const l=(stockLots||[]).find(x=>x.id===id);if(l){lotEdit(id);}else{$("#view").insertAdjacentHTML("afterbegin",`<div class="msg err">Tovar #${id} sa nenašiel (možno už bol vydaný).</div>`);}}
...
function renderRecv(){
  ...
  if(!recvSel){
    ...
    $("#view").innerHTML=`<div class="card">
      ...
      <div class="inline" style="justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap;margin:6px 0"><div>Aktuálna príjemka: <b class="mono">${currentReceiveDoc?esc(currentReceiveDoc):"— (pridelí sa pri prvom príjme)"}</b></div><button class="btn ghost sm" onclick="receiveNewDoc()">🆕 Nová príjemka</button></div>
      ...
      <div class="row2"><div><label>Stav</label><select id="r_state" onchange="rStatePreset()"><option value="new">nové</option><option value="used">použité</option><option value="refurb">repasované</option><option value="damaged">poškodené</option><option value="other">iné</option></select></div>
      ...
      <label>Zdroj ceny</label><select id="r_buysource">${BUY_SOURCES.map(s=>`<option>${esc(s)}</option>`).join("")}</select>
...
async function rSave(){
  ...
  if(track==="unit"&&sers.length>1){
    ...
    for(const s of sers){
      ...
      await sb.from("stock_movements").insert({type:"prijem",product_id:recvSel,lot_id:data.id,quantity:1,warehouse_id:lot.warehouse_id,location_id:loc,via:"fyzicky",document:lot.invoice_number,serial:lot.serial,doc_no:docNo});
    ...
    const {data,error}=await sb.from("stock_lots").insert(lot).select().single();
    if(error){$("#r_save").disabled=false;$("#r_msg").innerHTML=`<div class="msg err">${esc(error.message)}</div>`;return;}
    await sb.from("stock_movements").insert({type:"prijem",product_id:recvSel,lot_id:data.id,quantity:lot.quantity,warehouse_id:lot.warehouse_id,location_id:loc,via:"fyzicky",document:lot.invoice_number,serial:lot.serial,doc_no:docNo});
...
async function loadRecvHistory(){
  ...
  const {data,error}=await sb.from("stock_movements").select("id,quantity,serial,document,doc_no,happened_at,product_id,lot_id,warehouse_id,location_id,products(name),warehouses(name)").eq("type","prijem").order("happened_at",{ascending:false}).limit(40);
  ...
  el.innerHTML=`<div class="ptbl-wrap"><table class="ptbl"><thead><tr><th>Dátum</th><th>Príjemka</th><th>Produkt</th><th class="r">Ks</th><th>Sklad</th><th>SN</th><th>Doklad</th><th></th></tr></thead><tbody>`+
    data.map(m=>{const canDel=ME.role==="admin"||((Date.now()-new Date(m.happened_at).getTime())/60000)<=10;
      return `<tr><td>${esc((m.happened_at||"").replace("T"," ").slice(0,16))}</td><td class="mono" style="font-size:12px">${esc(m.doc_no||"—")}</td><td>${esc((m.products&&m.products.name)||"?")}</td><td class="r">+${fmtNum(m.quantity)}</td><td>${esc((m.warehouses&&m.warehouses.name)||"—")}</td><td>${esc(m.serial||"—")}</td><td>${esc(m.document||"—")}</td>
      <td style="white-space:nowrap">${m.doc_no?`<button class="btn ghost sm" data-tip="Pridať ďalší kus do tejto príjemky (${esc(m.doc_no)})" onclick="receiveContinue('${esc(m.doc_no)}')">➕ Pokračovať</button> `:""}<button class="btn ghost sm" onclick="recvDuplicate(${m.id})">⧉ Duplikovať</button>${canDel?` <button class="btn red sm" onclick="recvDelete(${m.id},${m.lot_id||"null"},'${esc(m.happened_at||"")}')">🗑</button>`:""}</td></tr>`;}).join("")+`</tbody></table></div>`;
}
...
async function renderIssue(){
  ...
  $("#view").innerHTML=`<div class="card"><h2>Výdaj</h2>
    ...
    <div class="inline" style="justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap">
      ...
      <button class="btn ghost sm" onclick="issueNewDoc()">🆕 Nová výdajka</button></div>
    <div class="row2" style="margin-top:8px"><div><label>Spôsob</label><select id="i_via"><option value="fyzicky">fyzicky</option><option value="zasielkou">zásielkou</option></select></div>
      <div><label>Dôvod výdaja</label><input id="i_reason" list="reasonList" placeholder="predaj / inventúra…"><datalist id="reasonList">${REASONS.map(r=>`<option>${esc(r)}</option>`).join("")}</datalist></div></div>
    <label>Doklad / objednávka (nepovinné)</label><input id="i_doc" placeholder="OBJ-…">
    <hr style="border:0;border-top:1px solid var(--line);margin:10px 0">
    <label>Pridať kus — nájdi (QR / SN / názov / pozícia) alebo skenuj</label>
    <div class="inline"><input id="i_q" placeholder="naskenuj QR alebo napíš…" oninput="iSearch()" autocomplete="off"><button class="btn ghost sm" onclick="openScan(t=>{$('#i_q').value=t;iSearch();},{qr:true})">📷 QR</button><button class="btn sm" onclick="issueScanAdd()">📷 Skenovať a pridať</button></div>
    <div id="i_sug" class="sug"></div>
    <div id="i_form"></div></div>
    <div class="card"><b style="font-size:14px">Na tejto výdajke</b><div id="i_current" style="margin-top:6px"></div></div>
  ...
  issueLots=(data||[]).filter(l=>l.status!=="na_ceste");iSearch();renderIssueCurrent();loadIssueHistory();
...
async function loadIssueHistory(){
  ...
  const {data,error}=await sb.from("stock_movements").select("id,quantity,serial,document,doc_no,purpose,happened_at,product_id,warehouse_id,location_id,products(name),warehouses(name)").eq("type","vydaj").order("happened_at",{ascending:false}).limit(40);
  if(error){el.innerHTML=`<span class="msg err">${esc(error.message)}</span>`;return;}
  if(!data||!data.length){el.textContent="Zatiaľ žiadne výdajky.";return;}
  el.innerHTML=`<div class="ptbl-wrap"><table class="ptbl"><thead><tr><th>Dátum</th><th>Výdajka</th><th>Produkt</th><th class="r">Ks</th><th>Sklad</th><th>SN</th><th>Doklad / účel</th><th></th></tr></thead><tbody>`+
    data.map(m=>`<tr><td>${esc((m.happened_at||"").replace("T"," ").slice(0,16))}</td><td class="mono" style="font-size:12px">${esc(m.doc_no||"—")}</td><td>${esc((m.products&&m.products.name)||"?")}</td><td class="r">−${fmtNum(m.quantity)}</td><td>${esc((m.warehouses&&m.warehouses.name)||"—")}</td><td>${esc(m.serial||"—")}</td><td>${esc(m.document||"")}${m.purpose?" · "+esc(m.purpose):""}</td>
...
// ===== VÝDAJ SA PRIEBEŽNE PLNÍ DO JEDNEJ VÝDAJKY (s číslom) =====
let currentIssueDoc=null,issueDocStartedAt=0;
async function ensureIssueDoc(){if(!currentIssueDoc){currentIssueDoc=await nextDocNo("vydaj");issueDocStartedAt=Date.now();const cd=$("#curdoc");if(cd)cd.textContent=currentIssueDoc||"—";}return currentIssueDoc;}
function issueNewDoc(){currentIssueDoc=null;issueDocStartedAt=0;const cd=$("#curdoc");if(cd)cd.textContent="— (pridelí sa pri prvom výdaji)";renderIssueCurrent();}
async function issueCommit(lot,qty){
  if(currentIssueDoc&&issueDocStartedAt&&!docAddAllowed(issueDocStartedAt)){alert("Do tejto vydajky uz nemozes pridavat — uplynul casovy limit pre tvoju rolu. Klikni 'Nova vydajka'.");return false;}
  await ensureIssueDoc();
  const via=($("#i_via")&&$("#i_via").value)||"fyzicky";
  const reason=($("#i_reason")&&$("#i_reason").value.trim())||null;
  const doc=($("#i_doc")&&$("#i_doc").value.trim())||null;
  const q=qty||lot.quantity;
  const {error}=await sb.from("stock_movements").insert({type:"vydaj",product_id:lot.product_id,lot_id:lot.id,quantity:q,warehouse_id:lot.warehouse_id,location_id:lot.location_id,via,document:doc,purpose:reason,serial:lot.serial,doc_no:currentIssueDoc});
  if(error){alert(error.message);return false;}
  if(lot.track==="bulk"&&q<lot.quantity){await sb.from("stock_lots").update({quantity:lot.quantity-q}).eq("id",lot.id);}
  else{await sb.from("stock_lots").delete().eq("id",lot.id);}
  return true;
}
async function issueRefresh(){
  const {data}=await sb.from("stock_lots").select("id,quantity,track,serial,qr_code,status,product_id,warehouse_id,location_id,products(name),warehouses(name),warehouse_locations(code)").order("id",{ascending:false}).limit(1000);
  issueLots=(data||[]).filter(l=>l.status!=="na_ceste");
  const q=$("#i_q");if(q)q.value="";const sug=$("#i_sug");if(sug)sug.innerHTML="";const form=$("#i_form");if(form)form.innerHTML="";issueSel=null;
  await renderIssueCurrent();loadIssueHistory();
}
function issueScanAdd(){openScan(async code=>{
  const c=String(code||"").trim().toLowerCase();if(!c)return;
  const lot=issueLots.find(l=>((l.qr_code||"").toLowerCase()===c)||((l.serial||"").toLowerCase()===c));
  if(!lot){alert("Tento kód sa nenašiel na sklade (alebo je už na ceste): "+code);return;}
  const ok=await issueCommit(lot,lot.quantity);if(ok)await issueRefresh();
},{qr:true});}
async function renderIssueCurrent(){
  const box=$("#i_current");if(!box)return;
  if(!currentIssueDoc){box.innerHTML=`<div class="muted" style="font-size:12px">Zatiaľ prázdna — pridaj prvý kus nižšie. Číslo výdajky sa pridelí pri prvom výdaji.</div>`;return;}
  const {data}=await sb.from("stock_movements").select("id,quantity,serial,products(name),warehouses(name)").eq("type","vydaj").eq("doc_no",currentIssueDoc).order("happened_at",{ascending:true});
  const items=data||[];
  if(!items.length){box.innerHTML=`<div class="muted" style="font-size:12px">Výdajka ${esc(currentIssueDoc)} — zatiaľ prázdna.</div>`;return;}
  box.innerHTML=items.map(m=>`<div class="lot" style="display:flex;justify-content:space-between;align-items:center"><div>${esc(m.products&&m.products.name||"?")} <span class="muted">${m.serial?"SN "+esc(m.serial)+" · ":""}−${fmtNum(m.quantity)} ks · ${esc(m.warehouses&&m.warehouses.name||"")}</span></div><button class="btn ghost sm" data-tip="Vrátiť na sklad (zrušiť túto položku)" onclick="issueUndo(${m.id})">↩</button></div>`).join("")+`<div class="muted" style="font-size:12px;margin-top:4px">Spolu ${items.length} položiek · výdajka <b>${esc(currentIssueDoc)}</b>.</div>`;
}
async function issueUndo(moveId){
  ...
  await sb.from("stock_movements").delete().eq("id",moveId);
  await issueRefresh();
}
// ===== PRÍJEM: číslo príjemky + priebežné plnenie / pokračovanie v existujúcej =====
let currentReceiveDoc=null;
async function ensureReceiveDoc(){if(!currentReceiveDoc)currentReceiveDoc=await nextDocNo("prijem");return currentReceiveDoc;}
function receiveNewDoc(){currentReceiveDoc=null;renderRecv();}
async function receiveContinue(docNo){if(!docNo)return;
  const {data}=await sb.from("stock_movements").select("happened_at").eq("doc_no",docNo).order("happened_at",{ascending:true}).limit(1);
  const oldest=data&&data[0]&&data[0].happened_at;
  if(!docAddAllowed(oldest)){alert("Do tejto príjemky už nemôžeš pridávať — pre tvoju rolu uplynul časový limit. Požiadaj admina.");return;}
  currentReceiveDoc=docNo;recvSel=null;setTab("recv");}
...
function iPick(id){issueSel=issueLots.find(l=>l.id===id);const l=issueSel;if(!l)return;
  $("#i_sug").innerHTML="";$("#i_q").value="";
  $("#i_form").innerHTML=`<div class="lot" style="display:flex;justify-content:space-between;align-items:center;margin-top:8px;gap:8px">
    <div><b>${esc(l.products&&l.products.name||"?")}</b><div class="muted">${esc((l.warehouses&&l.warehouses.name)||"")} · ${esc((l.warehouse_locations&&l.warehouse_locations.code)||"—")} · na sklade: ${fmtNum(l.quantity)}${l.serial?" · SN "+esc(l.serial):""}</div></div>
    <div class="inline" style="gap:6px;align-items:center">${l.track==="bulk"?`<input id="i_qty" type="number" min="1" max="${l.quantity}" value="${l.quantity}" style="max-width:80px" data-tip="Množstvo na výdaj">`:""}<button class="btn red" id="i_save" onclick="iDo()" style="width:auto;margin:0">+ Na výdajku</button></div></div><div id="i_msg"></div>`;
}
async function iDo(){
  const l=issueSel;if(!l)return;
  const qty=l.track==="bulk"?Math.max(1,Math.min(Number(($("#i_qty")&&$("#i_qty").value)||1),l.quantity)):1;
  const btn=$("#i_save");if(btn)btn.disabled=true;
  const ok=await issueCommit(l,qty);
...
function whSort(id){const w=DATA.warehouses.find(x=>String(x.id)===String(id));return (w&&w.sort_order!=null)?w.sort_order:9999;}
...
function renderStock(){
  ...
  keys.forEach(k=>{const ls=g[k];const p=prodOf(Number(k));const nm=(ls[0].products&&ls[0].products.name)||p.name||"?";
    ...
    const whIds=uniq(inStock.map(l=>l.warehouse_id).filter(x=>x!=null)).sort((a,b)=>whSort(a)-whSort(b)||String(whCode(a)).localeCompare(whCode(b)));
    const whChips=whIds.map(wid=>{const l=inStock.find(x=>x.warehouse_id===wid);return whChip(l);}).join("");
    const whGrid=whChips?`<div style="display:grid;grid-template-columns:repeat(2,max-content);gap:3px 4px">${whChips}</div>`:"";
    const onWayChip=onWayLots.length?`<div style="margin-top:3px"><span class="tag o" style="cursor:pointer" onclick="event.stopPropagation();stockOpenShipment(${(onWayLots.find(l=>l.shipment_id)||onWayLots[0]).shipment_id||"null"})">🚚 doručuje sa</span></div>`:"";
    const whCell=(whGrid+onWayChip)||"—";
    ...
    const inParen=[];if(onWay)inParen.push("+"+fmtNum(onWay));if(reserved)inParen.push("−"+fmtNum(reserved));
    const qtyCell=`<b>${fmtNum(onStock)}</b>${inParen.length?` <span class="tip muted" data-tip="Skladom ${fmtNum(onStock)} ks${onWay?" · +"+fmtNum(onWay)+" na ceste (objednané / na doručení)":""}${reserved?" · −"+fmtNum(reserved)+" rezervované":""}" style="cursor:help">(${inParen.join(" / ")})</span>`:""}`;
    const exp=!!stockExpanded[k];const allSel=ls.every(l=>stockSel[l.id]);
    const catp=esc(catPathText(p.category_id));
    body+=`<tr>
      <td style="width:16px;cursor:pointer;color:var(--grey);text-align:center" onclick="toggleExp('${k}')">${exp?"▾":"▸"}</td>
      <td><span style="cursor:pointer;display:inline-flex;align-items:center;gap:8px" onclick="toggleExp('${k}')">${th}<span><b class="pnm" style="cursor:pointer" onclick="event.stopPropagation();prodOpen(${k})">${esc(nm)}</b><div class="psub">${catp} · ${ls.length} šarží</div></span></span></td>
    ...
    if(exp){ls.forEach(l=>{const loc=(l.warehouse_locations&&l.warehouse_locations.code)||"—";
      const buySrc=(l.buy_source||l.buy_by)?` <span class="tip muted" data-tip="Zdroj ceny: ${esc(l.buy_source||"—")}${l.buy_by?" · zadal: "+esc(l.buy_by):""}" style="cursor:help">ⓘ</span>`:"";
      const buy=(l.buy_price!=null?fmtNum(l.buy_price)+" "+(l.buy_currency||""):"—")+(l.buy_date?" · "+String(l.buy_date).slice(0,10):"")+(l.invoice_number?" · "+esc(l.invoice_number):"")+buySrc;
  ...
  const table=keys.length?`<div class="ptbl-wrap"><table class="ptbl"><thead><tr><th></th><th>Produkt</th><th>Sklad</th><th>Pozícia</th><th class="r">Ks <span class="tip" data-tip="Množstvo na sklade. Zátvorka: + na ceste (objednané / na doručení) · − rezervované" style="cursor:help;color:var(--grey);font-weight:400">ⓘ</span></th><th>Stav</th><th>Nákup (cena / dátum / faktúra)</th><th></th></tr></thead><tbody>${body}</tbody></table></div>`:`<div class="muted">Žiadne zásoby pre daný filter.</div>`;
  ...
  <div class="card">
    <div class="inline" style="gap:8px;flex-wrap:wrap;align-items:center;justify-content:space-between;margin-bottom:8px">
      <div><b style="font-size:15px">Zásoby</b> <span class="muted" style="font-size:12px">${keys.length} produktov · ${lots.length} šarží · ${fmtNum(totKs)} ks</span></div>
      <div class="inline" style="gap:6px;flex-wrap:wrap">
        <button class="btn ghost sm" onclick="stockScanFind()">📷 Nájsť tovar (QR/SN)</button>
    ...
    <div class="toolbar"><input id="stockQ" placeholder="Hľadať produkt / SN / QR / pozíciu…" value="${esc(sf.q)}" oninput="sf.q=this.value;searchInput('stockQ',()=>{saveStockFilters();renderStock()})" style="flex:2 1 220px">
      ...
      <select onchange="sf.state=this.value;saveStockFilters();renderStock()" style="flex:0 1 120px">${stateOpts}</select>
      <select onchange="sf.brand=this.value;saveStockFilters();renderStock()" style="flex:0 1 130px">${brOpts}</select></div>
...
async function bulkIssueDo(){const ids=selectedLotIds();const items=ids.map(id=>stockLots.find(l=>l.id===id)).filter(Boolean).filter(l=>l.status!=="na_ceste");
  ...
  const docNo=await nextDocNo("vydaj");
  const mv=items.map(l=>({type:"vydaj",product_id:l.product_id,lot_id:l.id,quantity:l.quantity,warehouse_id:l.warehouse_id,location_id:l.location_id,via,document:doc,purpose:reason,serial:l.serial,doc_no:docNo}));
  ...
  $("#view").insertAdjacentHTML("afterbegin",`<div class="msg ok">✓ Vydané ${items.length} položiek na jednej výdajke <b>${esc(docNo||"—")}</b>${doc?" ("+esc(doc)+")":""}.</div>`);
}
...
function lotEdit(id){const l=stockLots.find(x=>x.id===id);if(!l)return;
  ...
  openModal(`<div style="display:flex;justify-content:space-between;align-items:center"><h2>Upraviť tovar</h2><div class="inline" style="gap:6px">${copyLinkBtn("lot/"+id)}<button class="btn ghost sm" onclick="closeModal()">✕</button></div></div>
    ...
    <div><label>Sériové číslo</label><div class="inline" style="gap:6px"><input id="le_sn" value="${esc(l.serial||"")}"><button class="btn ghost sm" type="button" title="Odfotiť a rozpoznať SN (AI)" onclick="lotSnPhoto()">📷</button></div><div id="le_snmsg" class="muted" style="font-size:11.5px;margin-top:2px"></div></div></div>
    ...
    <label>Zdroj ceny</label><select id="le_buysource"><option value="">—</option>${BUY_SOURCES.map(s=>`<option ${l.buy_source===s?"selected":""}>${esc(s)}</option>`).join("")}</select>
    ${l.buy_by?`<div class="muted" style="font-size:11.5px;margin-top:3px">Cenu zadal: <b>${esc(l.buy_by)}</b>${l.buy_source?" · zdroj: "+esc(l.buy_source):""}</div>`:""}
...
async function lotSnPhoto(){
  ...
  inp.onchange=async()=>{const f=inp.files&&inp.files[0];if(!f)return;const box=$("#le_snmsg");if(box)box.textContent="🔍 Rozpoznávam SN…";
    try{
      const blob=await compressImage(f,900000,2000);
      ...
      if(error||!data||data.error){if(box)box.textContent="Rozpoznanie zlyhalo: "+((data&&data.error)||(error&&error.message)||"?");return;}
      const sn=(data.serial||"").trim();const el=$("#le_sn");
      if(sn&&el){el.value=sn;if(box)box.innerHTML=`✓ SN: <b>${esc(sn)}</b>`;}
      else if(box)box.textContent="SN sa na fotke nenašlo — skús bližší záber štítku.";
    }catch(e){if(box)box.textContent="Chyba: "+(e.message||e);}
  };inp.click();
}
async function lotEditSave(id){
  const o={warehouse_id:Number($("#le_wh").value),location_id:$("#le_loc").value?Number($("#le_loc").value):null,
    ...
    buy_source:($("#le_buysource")&&$("#le_buysource").value)||null,buy_by:ME.email||null,
...
function renderPlacement(){
  ...
  openModal(`<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px"><h2>Rozmiestnenie skladu</h2><button class="btn ghost sm" onclick="closeModal()">✕</button></div>
    ...
    ${w?`<div class="row2" style="margin-top:10px"><div><label>Názov (krátky)</label><input value="${esc(w.name)}" onchange="plRenameWh(this.value)" placeholder="napr. Rostovská"></div>
      <div><label>Skratka (kód) · poradie zobrazenia</label><div class="inline" style="gap:6px"><input value="${esc(w.code||'')}" maxlength="4" onchange="plSetCode(this.value)" placeholder="ROS" style="max-width:90px;text-transform:uppercase"><input type="number" value="${w.sort_order!=null?w.sort_order:''}" onchange="plSetOrder(this.value)" placeholder="poradie" style="max-width:90px" data-tip="Poradie skladu v stĺpci Sklad (menšie číslo = skôr)"></div></div></div>
...
async function plSetOrder(v){const n=v===""?null:Number(v);const {error}=await sb.from("warehouses").update({sort_order:n}).eq("id",Number(plWh));if(error){alert(error.message);return;}await loadData();renderPlacement();}
...
function prodDetail(id){const p=DATA.products.find(x=>x.id===id);if(!p)return;navHash("prods/"+id);
  ...
  const missingParams=!p.long_description&&params.length===0;
  $("#view").innerHTML=`<div class="card"><div class="chosen"><b>${esc(p.name)}</b><div style="display:flex;gap:6px;flex-wrap:wrap">${canWrite()?`<button class="btn ghost sm" onclick="prodForm(${id})">✏️ Upraviť</button><button class="btn ghost sm" onclick="prodChangePrice(${id})">💰 Zmeniť cenu</button><button class="btn ghost sm" onclick="prodFetchSpecs(${id})">🔎 Dohľadať parametre</button><button class="btn ghost sm" onclick="prodPrintQR(${id})">🏷️ Tlač QR</button>`:""}${copyLinkBtn("prods/"+id)}<button class="btn ghost sm" onclick="setTab(tab==='prods'?'prods':'stock')">Späť</button></div></div>
    ${missingParams?`<div class="msg" style="background:var(--warn-bg);color:var(--warn-tx)">Chýbajú parametre. ${p.sku?"Skús tlačidlo Dohľadať parametre.":"Doplň EAN a potom dohľadaj parametre, alebo vyplň ručne cez Upraviť."}</div>`:""}
    <div style="display:flex;gap:14px;flex-wrap:wrap;align-items:flex-start;margin-top:10px">
      <div>${p.image_url?`<img src="${esc(p.image_url)}" style="width:150px;height:150px;object-fit:contain;border-radius:10px;border:1px solid var(--line);background:var(--card)">`:`<div style="width:150px;height:150px;background:var(--bg-2);border-radius:10px;display:flex;align-items:center;justify-content:center" class="muted">bez fotky</div>`}</div>
...
async function loadDocs(){
  $("#view").innerHTML=`<div class="card muted">Načítavam…</div>`;
  const cols="id,type,quantity,serial,document,doc_no,purpose,via,happened_at,product_id,warehouse_id,location_id,user_id,products(name),warehouses(name),warehouse_locations(code)";
...
function renderDocs(){
  ...
  const rowH=m=>{const prijem=m.type==="prijem";const nm=(m.products&&m.products.name)||"?";
    ...
    return `<tr onclick="${m.product_id?`prodDetail(${m.product_id})`:""}" style="cursor:pointer;background:${prijem?"#f1faf4":"#fdf2f2"}">
      ...
      <td>${esc(usr)}</td>
      <td class="mono" style="font-size:12px">${esc(m.doc_no||"—")}</td>
      <td>${m.document?esc(m.document):"—"}</td>
      <td>${esc(m.via||"")}${m.purpose?`<div class="psub">${esc(m.purpose)}</div>`:""}</td></tr>`;};
  const table=list.length?`<div class="ptbl-wrap"><table class="ptbl"><thead><tr><th>Dátum a čas</th><th>Typ</th><th>Produkt</th><th class="r">Množstvo</th><th>Sklad / Pozícia</th><th>Používateľ</th><th>Č. dokladu</th><th>Doklad</th><th>Spôsob / účel</th></tr></thead><tbody>${list.map(rowH).join("")}</tbody></table></div>`:`<div class="muted">Žiadne pohyby.</div>`;
  $("#view").innerHTML=`
...
// rozšírené filtre + zoradenie
let shipCarrierF="", shipStatusF="", shipCountryF="", shipCostMin="", shipCostMax="", shipCreatedFrom="", shipCreatedTo="", shipDelivFrom="", shipDelivTo="", shipSort="new";
let shipExpandId=null; // ktorá zásielka je rozbalená priamo v zozname
// prepínanie zobrazenia stĺpcov (uložené lokálne)
const SHIP_COL_DEFS=[{k:"pay",l:"Objednávka / platba"},{k:"cost",l:"Doprava (cena)"},{k:"customs",l:"Colné (AWB/JDS)"}];
function shipCols(){try{return Object.assign({pay:true,cost:true,customs:true},JSON.parse(localStorage.getItem("shipcols_"+ME.id)||"{}"));}catch(e){return {pay:true,cost:true,customs:true};}}
function shipColToggle(k){const c=shipCols();c[k]=!c[k];localStorage.setItem("shipcols_"+ME.id,JSON.stringify(c));shipList();}
// nápoveda pri skratkách: podčiarknuté bodkovane, tooltip s plným názvom
function abbr(short,full){return `<span title="${esc(full)}" style="text-decoration:underline dotted;text-underline-offset:2px;cursor:help">${esc(short)}</span>`;}
// ZJEDNOTENÝ stav (raw stav dopravcu ostáva v tooltipe)
function shipUni(s){
  if(shipDelivered(s))return {l:"Doručené",c:"g"};
  if(shipProblem(s))return {l:"Rieši sa",c:"r"};
  if(shipStageRank(s)>=1)return {l:"Na ceste",c:"b"};
  return {l:"Zadané",c:""};
}
function shipUniPill(s){const u=shipUni(s);const raw=s.status?` title="Stav dopravcu: ${esc(s.status)}"`:"";return `<span class="tag ${u.c}"${raw}>${esc(u.l)}</span>`;}
// cieľová krajina (posledný diel adresy) — na filter aj zobrazenie
function shipDestCountry(s){const t=(s.to_address||"").trim();if(!t)return "";const parts=t.split(",").map(x=>x.trim()).filter(Boolean);const last=parts[parts.length-1]||"";return last.length<=24?last:"";}
// zamknutie zásielky (manuálne; automatické pravidlá doplníme neskôr). Po zamknutí nemožno meniť poznámky (okrem admina).
function shipCanEdit(s){return canWrite()&&(!s.locked||ME.role==="admin");}
async function shipToggleLock(id,locked){
  if(locked&&ME.role!=="admin"&&!canWrite())return;
  if(locked){if(!confirm("Odomknúť zásielku? Po odomknutí bude opäť možné meniť údaje."))return;}
  else{if(!confirm("Zamknúť zásielku? Po zamknutí sa poznámky/údaje nebudú dať meniť (okrem admina)."))return;}
  const upd=locked?{locked:false,locked_at:null,locked_by:null}:{locked:true,locked_at:new Date().toISOString(),locked_by:ME.email||ME.id};
  const {error}=await sb.from("shipments").update(upd).eq("id",id);
  if(error){alert(error.message);return;}
  if(shipMode==="detail"&&shipDetailId===id)shipDetail(id);else shipList();
}
...
async function shipList(){
  $("#view").innerHTML=`<div class="card muted">Načítavam…</div>`;
  const {data,error}=await sb.from("shipments").select("id,tracking_number,carrier,direction,status,sender,from_address,to_address,our_order,order_source,contents,descr,expected_date,delivered_on,created_at,label_date,is_paid,payment_method,paid_date,customs,incoterm,jds_number,awb_number,customs_value,duty,vat,invoice_number,customer_payment,paid_where,pay_amount,pay_currency,ship_cost,ship_cost_cur,locked").order("id",{ascending:false}).limit(500);
  if(error){$("#view").innerHTML=`<div class="card"><div class="msg err">${esc(error.message)}</div></div>`;return;}
  let list=data||[];
  ...
  // zoznam prepravcov a krajín pre výber vo filtroch (z reálnych dát)
  const carriersAll=[...new Set(list.map(s=>detectCarrierName(s.carrier,s.tracking_number)||s.carrier).filter(Boolean))].sort();
  const countriesAll=[...new Set(list.map(shipDestCountry).filter(Boolean))].sort();
  ...
  if(shipCarrierF)list=list.filter(s=>(detectCarrierName(s.carrier,s.tracking_number)||s.carrier)===shipCarrierF);
  if(shipStatusF)list=list.filter(s=>shipUni(s).l===shipStatusF);
  if(shipCountryF)list=list.filter(s=>shipDestCountry(s)===shipCountryF);
  if(shipCostMin!=="")list=list.filter(s=>Number(s.ship_cost||0)>=Number(shipCostMin));
  if(shipCostMax!=="")list=list.filter(s=>Number(s.ship_cost||0)<=Number(shipCostMax));
  const cdate=s=>String(s.label_date||s.created_at||"").slice(0,10);
  const ddate=s=>String(s.delivered_on||s.expected_date||"").slice(0,10);
  if(shipCreatedFrom)list=list.filter(s=>cdate(s)&&cdate(s)>=shipCreatedFrom);
  if(shipCreatedTo)list=list.filter(s=>cdate(s)&&cdate(s)<=shipCreatedTo);
  if(shipDelivFrom)list=list.filter(s=>ddate(s)&&ddate(s)>=shipDelivFrom);
  if(shipDelivTo)list=list.filter(s=>ddate(s)&&ddate(s)<=shipDelivTo);
  if(shipQ){const q=shipQ.toLowerCase();list=list.filter(s=>((s.tracking_number||"")+" "+(s.carrier||"")+" "+(s.sender||"")+" "+(s.to_address||"")+" "+(s.from_address||"")+" "+(s.contents||"")+" "+(s.descr||"")+" "+(s.our_order||"")+" "+(s.invoice_number||"")+" "+(s.jds_number||"")+" "+(s.awb_number||"")).toLowerCase().includes(q));}
  // zoradenie
  const num=x=>Number(x||0);
  const SORTF={new:(a,b)=>b.id-a.id,old:(a,b)=>a.id-b.id,
    eta:(a,b)=>(ddate(a)||"9999").localeCompare(ddate(b)||"9999"),
    created:(a,b)=>(cdate(b)||"").localeCompare(cdate(a)||""),
...
async function shipExpandFill(id){const el=$("#shipexp_"+id);if(!el)return;
  const {data:s}=await sb.from("shipments").select("*").eq("id",id).single();
  if(!s){el.innerHTML=`<span class="muted">Nedá sa načítať.</span>`;return;}
  const {data:its}=await sb.from("shipment_items").select("id,quantity,product_id,products(name)").eq("shipment_id",id);
  const {data:lots}=await sb.from("stock_lots").select("id,status").eq("shipment_id",id);
  el.innerHTML=`<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px"><b>Detail zásielky</b><button class="btn ghost sm" onclick="shipDetail(${id})">⤢ Otvoriť v samostatnom okne</button></div>`+shipDetailHTML(s,its||[],lots||[]);
}
...
const CARRIER_INFO=[
  {name:"UPS",bg:"#5a3410",fg:"#fff",access:"napojené (API)",accessCls:"g",
   ...
   label:["<b>Tracking:</b> 1Z + 16 znakov (spolu 18) — hlavný kód na skenovanie","<b>Reference No.</b> — číslo od odosielateľa (obj./PO)","MI kód pri UPS Mail Innovations"],
  ...
  {name:"FedEx",bg:"#4d148c",fg:"#fff",access:"napojené (API)",accessCls:"g",
   ...
   label:["<b>Tracking:</b> 12 číslic (Express/Ground)","<b>Door tag</b> DT + 12 číslic (neúspešné doručenie)","RMA / referencia od odosielateľa"],
  ...
  {name:"DHL",bg:"#d40511",fg:"#ffcc00",access:"napojené (API)",accessCls:"g",
   ...
   label:["<b>Waybill / tracking:</b> 10 číslic (Express), príp. JJD/JVGL/JD…","<b>AWB</b> (letecký, freight): XXX-XXXXXXXX (11 číslic)","Smerový kód (napr. CZ-PRG-GTW) = NIE tracking; Piece ID"],
  ...
  {name:"GLS",bg:"#061ab1",fg:"#ffd100",access:"napojené (API)",accessCls:"g",
   ...
   label:["<b>Parcel No.:</b> 11–14 číslic (hlavný kód)","Track ID / ZBA…","Referencia odosielateľa"],
  ...
  {name:"Zásilkovna",bg:"#bd2c30",fg:"#fff",access:"pripravené (API)",accessCls:"o",
   ...
   label:["<b>Číslo zásielky:</b> Z + 9–10 číslic","Kód výdajného miesta","Externý tracking nadväzujúceho dopravcu"],
  ...
  {name:"Česká pošta / Balíkovna",bg:"#e2001a",fg:"#ffdd00",access:"verejné API — bez kľúča",accessCls:"g",
   ...
   label:["<b>Podacie číslo:</b> 2 písmená + 9 číslic + 2 písmená (napr. RR…CZ)","<b>Balíkovna:</b> DR…C","Čiarový kód dobierky (poukaz)"],
  ...
  {name:"Neznámy / Ázia (agregátor)",bg:"#2a2621",fg:"#d9822b",access:"odporúčané",accessCls:"",
   ...
   label:["Formát čísla určí dopravcu automaticky","Čínske: faktúra '单号', HAWB pri leteckých"],
...
function shipCarrierInfo(){
  ...
  const liH=a=>(a||[]).map(x=>`<li>${x}</li>`).join(""); // label = dôveryhodný HTML (<b>)
  const blocks=CARRIER_INFO.map(c=>`
    <div style="border:1px solid var(--line);border-radius:10px;padding:12px;margin-bottom:10px;background:var(--card)">
      ...
      <div class="row2" style="gap:14px;flex-wrap:wrap">
        <div style="min-width:200px"><div class="muted" style="font-weight:700;text-transform:uppercase;font-size:11px;letter-spacing:.04em;margin-bottom:4px">📡 Cez API (tracking)</div><ul style="margin:0;padding-left:18px;font-size:13px;line-height:1.6">${li(c.api)}</ul></div>
        <div style="min-width:200px"><div class="muted" style="font-weight:700;text-transform:uppercase;font-size:11px;letter-spacing:.04em;margin-bottom:4px">🔐 Z portálu po prihlásení</div><ul style="margin:0;padding-left:18px;font-size:13px;line-height:1.6">${li(c.portal)}</ul></div>
        <div style="min-width:200px"><div class="muted" style="font-weight:700;text-transform:uppercase;font-size:11px;letter-spacing:.04em;margin-bottom:4px">🏷️ Kódy na štítku</div><ul style="margin:0;padding-left:18px;font-size:13px;line-height:1.6">${liH(c.label)}</ul></div>
      </div>
  ...
  openModal(`<div style="display:flex;justify-content:space-between;align-items:center"><h2>Dopravcovia — čo vieme zistiť + kódy na štítku</h2><button class="btn ghost sm" onclick="closeModal()">✕</button></div>
    <div class="muted" style="margin-bottom:10px">Pri každom prepravcovi: čo vráti <b>tracking API</b> (automaticky), čo nájdeš <b>na portáli po prihlásení</b> (ručne) a <b>čo znamenajú kódy na štítku</b>.</div>
    <div class="msg" style="background:var(--acc-soft);color:#8a4e14">📷 <b>AI rozpoznávanie štítku:</b> pri príjme zásielky odfoť štítok a appka sama rozlíši <b>tracking</b> · <b>AWB</b> · <b>colné (JDS/MRN)</b> · <b>faktúru</b> · <b>sériové číslo</b> · EAN a predvyplní ich.</div>
    <div class="msg" style="background:var(--warn-bg);color:var(--warn-tx);margin-top:6px">⚠️ <b>Cenu prepravy</b> (čo si reálne zaplatil) nevracia žiadny tracking — len fakturačné portály alebo faktúra dodávateľa.</div>
...
async function shipDetail(id){navHash("ship/"+id);shipMode="detail";shipDetailId=id;
  const {data:s,error}=await sb.from("shipments").select("*").eq("id",id).single();
  if(error){alert(error.message);return;}
  const {data:its}=await sb.from("shipment_items").select("id,quantity,product_id,products(name)").eq("shipment_id",id);
  const {data:lots}=await sb.from("stock_lots").select("id,status").eq("shipment_id",id);
  const d=DIRS[s.direction]||["?",""];const canEdit=shipCanEdit(s);
  $("#view").innerHTML=`<div class="card">
    <div class="chosen"><div><span class="tag ${d[1]}">${d[0]}</span> <b>${esc(s.tracking_number)}</b>${copyBtn(s.tracking_number)}${s.locked?` <span title="Zamknuté" style="cursor:help">🔒</span>`:""}</div>
      <div style="display:flex;gap:6px;flex-wrap:wrap">${(canWrite()&&!shipClosed(s))?`<button class="btn sm" onclick="shipUpsTrack(${id})">🔄 Zistiť stav u prepravcu</button>`:""}<button class="btn ghost sm" onclick="shipPOD(${id})">🧾 ${abbr("POD","Proof of Delivery — potvrdenie o doručení")}</button>${canWrite()?`<button class="btn ghost sm" onclick="shipToggleLock(${id},${s.locked?"true":"false"})">${s.locked?"🔓 Odomknúť":"🔒 Zamknúť"}</button>`:""}${copyLinkBtn("ship/"+id)}<button class="btn ghost sm" onclick="shipMode='list';shipList()">← Späť na zoznam</button></div></div>
    ${s.locked?`<div class="msg" style="background:var(--warn-bg);color:var(--warn-tx)">🔒 <b>Zamknuté</b>${s.locked_at?" · "+esc(String(s.locked_at).replace("T"," ").slice(0,16)):""}${s.locked_by?" · "+esc(s.locked_by):""} — poznámky a údaje nie je možné meniť${ME.role==="admin"?" (ako admin môžeš).":"."}</div>`:""}
    <div id="ship_upsmsg"></div>
    ${shipDetailHTML(s,its||[],lots||[])}
    ${canDelete()&&canEdit?`<button class="btn red" onclick="shipDelete(${id})" style="margin-top:14px">🗑 Zmazať zásielku</button>`:""}
  </div>`;
}
// úprava fyzických parametrov + popisu (rýchlo cez prompt; plný editačný formulár doplníme)
async function shipEditMeta(id){
  const {data:s}=await sb.from("shipments").select("weight,dimensions,descr,locked").eq("id",id).single();
  if(!s)return;if(s.locked&&ME.role!=="admin"){alert("Zásielka je zamknutá.");return;}
  const weight=prompt("Hmotnosť (napr. 12,5 kg):",s.weight||"");if(weight===null)return;
  const dimensions=prompt("Rozmery (napr. 60×40×40 cm):",s.dimensions||"");if(dimensions===null)return;
  const descr=prompt("Popis (napr. nebezpečný materiál / ADR) — vyhľadateľné:",s.descr||"");if(descr===null)return;
  const {error}=await sb.from("shipments").update({weight:weight.trim()||null,dimensions:dimensions.trim()||null,descr:descr.trim()||null}).eq("id",id);
  if(error){alert(error.message);return;}
  if(shipMode==="detail"&&shipDetailId===id)shipDetail(id);else{shipExpandId=id;shipList();}
}
// po úprave detailu: obnov buď samostatné okno, alebo rozbalený riadok v zozname
function shipRerender(sid){if(shipMode==="detail"&&shipDetailId){shipDetail(shipDetailId);}else{if(sid)shipExpandId=sid;shipList();}}
async function shipItemAdd(id){const pe=$("#si_prod_"+id)||$("#si_prod");const qe=$("#si_qty_"+id)||$("#si_qty");const pid=Number(pe&&pe.value);const qty=Number((qe&&qe.value)||1);if(!pid)return;
  const {error}=await sb.from("shipment_items").insert({shipment_id:id,product_id:pid,quantity:qty});if(error){alert(error.message);return;}shipRerender(id);}
async function shipItemDel(itemId){const sid=shipMode==="detail"?shipDetailId:shipExpandId;const {error}=await sb.from("shipment_items").delete().eq("id",itemId);if(error){alert(error.message);return;}shipRerender(sid);}
...
async function shipTogglePaid(id,cur){const o={is_paid:!cur};if(!cur){if(!confirm("Označiť zásielku ako zaplatenú?"))return;o.paid_date=new Date().toISOString().slice(0,10);}else{o.paid_date=null;}const {error}=await sb.from("shipments").update(o).eq("id",id);if(error){alert(error.message);return;}shipRerender(id);}
...
let repKind="",repStatus="",repQ="",repTech="",repSort="new",repWaiting=false,repPhotos=[];
// SLA / zvýraznenie čakajúcich (dni) — uložené lokálne
function repSla(){try{return Object.assign({def:5,diagnostika:2,partsAfter:3},JSON.parse(localStorage.getItem("repsla_"+ME.id)||"{}"));}catch(e){return {def:5,diagnostika:2,partsAfter:3};}}
function repDays(ts){if(!ts)return 0;return Math.floor((Date.now()-new Date(ts).getTime())/86400000);}
function repBreach(r){if(!r||r.status==="uzavrete")return null;const s=repSla();
  if(r.status==="cakanie_diely"&&r.parts_expected){const over=Math.floor((Date.now()-new Date(r.parts_expected).getTime())/86400000)-s.partsAfter;return over>0?{days:over,reason:"diely meškajú (nad termín + "+s.partsAfter+" dní)"}:null;}
  const lim=r.status==="diagnostika"?s.diagnostika:s.def;const d=repDays(r.updated_at||r.created_at);
  return d>lim?{days:d,reason:"čaká v stave nad "+lim+" dní"}:null;}
// stavy, ktoré nastávajú len ak sa reálne opravuje (pri „neopravuje sa" sú neaplikovateľné)
const REP_REPAIR_ONLY=new Set(["k_oprave","cakanie_diely","opravene"]);
function repStepBar(r){const status=r.status;const cur=repStageNum(status);const na=!!r.not_repaired;
  return `<div style="display:flex;gap:2px;min-width:110px" data-tip="${esc(repStageLabel(status))}${na?" · NEOPRAVUJE SA":""}">${REP_STAGES.map(s=>{
    if(na&&REP_REPAIR_ONLY.has(s.k))return `<span data-tip="${esc(s.label)} — neaplikovateľné (neopravuje sa)" style="flex:1;height:6px;border-radius:3px;background:repeating-linear-gradient(45deg,var(--line-2),var(--line-2) 2px,transparent 2px,transparent 4px)"></span>`;
    const isCur=s.n===cur;const on=s.n<cur;return `<span style="flex:1;height:6px;border-radius:3px;background:${isCur?"var(--blue)":(on?"var(--green)":"var(--line-2)")}"></span>`;}).join("")}</div>`;}
function repSlaModal(){const s=repSla();
  openModal(`<div style="display:flex;justify-content:space-between;align-items:center"><h2>Termíny / zvýraznenie</h2><button class="btn ghost sm" onclick="closeModal()">✕</button></div>
    <div class="muted" style="font-size:12.5px;margin-bottom:8px">Zvýrazniť opravu, ak čaká dlhšie ako:</div>
    <div class="row2"><div><label>Bežne v stave (dni)</label><input id="sla_def" type="number" value="${s.def}"></div>
      <div><label>V stave Diagnostika (dni)</label><input id="sla_diag" type="number" value="${s.diagnostika}"></div></div>
    <label>Čakanie na diely — po predpokladanom termíne (dni)</label><input id="sla_parts" type="number" value="${s.partsAfter}">
    <div style="text-align:right;margin-top:12px"><button class="btn" style="width:auto" onclick="repSlaDo()">Uložiť</button></div>`);}
function repSlaDo(){const o={def:Number($("#sla_def").value)||5,diagnostika:Number($("#sla_diag").value)||2,partsAfter:Number($("#sla_parts").value)||3};localStorage.setItem("repsla_"+ME.id,JSON.stringify(o));closeModal();renderRepairs();}
function renderRepairs(){
  if(!canWrite()){$("#view").innerHTML=`<div class="card"><div class="msg err">Táto sekcia je pre user/admin.</div></div>`;return;}
  $("#view").innerHTML=`<div class="card muted">Načítavam…</div>`;
  sb.from("repairs").select("*").order("id",{ascending:false}).limit(1000).then(({data,error})=>{
    if(error){$("#view").innerHTML=`<div class="card"><div class="msg err">${esc(error.message)}</div></div>`;return;}
    let list=data||[];
    const techs=[...new Set(list.map(r=>r.technician).filter(Boolean))].sort();
    if(repKind)list=list.filter(r=>r.kind===repKind);
    if(repStatus)list=list.filter(r=>r.status===repStatus);
    if(repTech)list=list.filter(r=>(r.technician||"")===repTech);
    if(repWaiting)list=list.filter(r=>repBreach(r));
    if(repQ){const q=repQ.toLowerCase();list=list.filter(r=>((r.title||"")+" "+(r.repair_no||"")+" "+(r.customer||"")+" "+(r.customer_email||"")+" "+(r.customer_phone||"")+" "+(r.serial||"")+" "+(r.technician||"")+" "+(r.last_by||"")+" "+(r.received_by||"")).toLowerCase().includes(q));}
    const SORT={new:(a,b)=>b.id-a.id,old:(a,b)=>a.id-b.id,deadline:(a,b)=>(a.deadline||"9999").localeCompare(b.deadline||"9999"),waiting:(a,b)=>repDays(b.updated_at||b.created_at)-repDays(a.updated_at||a.created_at)};
    list=list.slice().sort(SORT[repSort]||SORT.new);
    const nm=r=>r.title||((DATA.products.find(p=>p.id===r.product_id)||{}).name)||"(bez názvu)";
    const today=new Date().toISOString().slice(0,10);
    const rowH=r=>{const br=repBreach(r);const overdue=r.kind==="reklamacia"&&r.deadline&&r.deadline<today&&r.status!=="uzavrete";
      const hl=br?(br.days>=repSla().def*2?"background:#fdeeee":"background:#fff6e9"):"";
      return `<tr onclick="repairDetail(${r.id})" style="cursor:pointer;${hl}">
        <td style="max-width:190px"><b>${esc(nm(r))}</b>${r.repair_no?`<div class="psub mono">${esc(r.repair_no)}</div>`:""}${r.serial?`<div class="psub">SN: ${esc(r.serial)}</div>`:""}${r.kind==="reklamacia"?`<span class="tag r" style="font-size:10px">reklamácia</span>`:""}</td>
        <td>${esc(r.customer||"—")}${r.customer_contact?`<div class="psub">${esc(r.customer_contact)}</div>`:""}</td>
        <td>${repStepBar(r)}<div class="psub"><span class="tag ${repStatusColor(r.status)}" style="font-size:10px">${repStageNum(r.status)} · ${esc(repStageLabel(r.status).replace(/^\d+ · /,""))}</span>${r.not_repaired?` <span class="tag r" style="font-size:10px">❌ neopravuje sa${r.disposition?" · "+esc(r.disposition):""}</span>`:""}</div></td>
        <td>${br?`<span class="tag o" data-tip="${esc(br.reason)}">⏱ ${br.days} dní</span>`:`<span class="muted">${repDays(r.updated_at||r.created_at)} dní</span>`}${overdue?`<div class="psub"><span class="tag r">po termíne ${esc(String(r.deadline).slice(0,10))}</span></div>`:(r.deadline?`<div class="psub">termín ${esc(String(r.deadline).slice(0,10))}</div>`:"")}</td>
        <td><b>${esc(r.technician||"—")}</b>${r.last_by?`<div class="psub">zmenil: ${esc(r.last_by)}</div>`:""}${r.received_by?`<div class="psub">prijal: ${esc(r.received_by)}</div>`:""}</td>
        <td>${esc(String(r.created_at||"").slice(0,10))}</td></tr>`;};
    const thead=`<thead><tr><th>Tovar</th><th>Zákazník</th><th>Stav</th><th>Čaká / termín</th><th>Technik / zmenil</th><th>Vytvorené</th></tr></thead>`;
    let sections="";
    if(!list.length){sections=`<div class="card"><div class="muted">Žiadne záznamy pre daný filter.</div></div>`;}
    else{
      REP_STAGES.forEach(st=>{const grp=list.filter(r=>r.status===st.k);if(!grp.length)return;
        const brc=grp.filter(r=>repBreach(r)).length;
        sections+=`<div class="card"><h3 style="margin:0 0 8px"><span class="tag ${repStatusColor(st.k)}">${esc(st.label)}</span> <span class="muted" style="font-weight:400">${grp.length}</span>${brc?` <span class="tag o">⏱ ${brc} čaká</span>`:""}</h3>
          <div class="ptbl-wrap"><table class="ptbl">${thead}<tbody>${grp.map(rowH).join("")}</tbody></table></div></div>`;});
      const other=list.filter(r=>!REP_STATUS.includes(r.status));
      if(other.length)sections+=`<div class="card"><h3 style="margin:0 0 8px">Iné (${other.length})</h3><div class="ptbl-wrap"><table class="ptbl">${thead}<tbody>${other.map(rowH).join("")}</tbody></table></div></div>`;
    }
    ...
    const so=(v,l)=>`<option value="${v}" ${repSort===v?"selected":""}>${l}</option>`;
    $("#view").innerHTML=`
    <div class="card">
      <div class="inline" style="gap:8px;flex-wrap:wrap;align-items:center;justify-content:space-between;margin-bottom:8px">
        <div><b style="font-size:15px">Opravy a reklamácie</b> <span class="muted" style="font-size:12px">${list.length} záznamov</span></div>
        <div class="inline" style="gap:6px;flex-wrap:wrap">
          <button class="btn green sm" onclick="repairForm(0,'oprava')">+ Oprava</button>
          <button class="btn red sm" onclick="repairForm(0,'reklamacia')">+ Reklamácia</button>
          <button class="btn ghost sm" onclick="repSlaModal()">⏱ Termíny / zvýraznenie</button></div>
      </div>
      <div style="border-top:1px solid var(--line-2);margin-bottom:8px"></div>
      <div class="toolbar"><input id="repQ" placeholder="🔎 Hľadať — číslo opravy (OPR…), zákazník, SN, tel./e-mail, technik…" value="${esc(repQ)}" oninput="repQ=this.value;searchInput('repQ',renderRepairs)" style="flex:2 1 240px">
        <select onchange="repSort=this.value;renderRepairs()">${so("new","Najnovšie")}${so("old","Najstaršie")}${so("deadline","Podľa termínu")}${so("waiting","Najdlhšie čaká")}</select></div>
      <div class="toolbar">
        ...
        <select onchange="repStatus=this.value;renderRepairs()">${stOpts}</select>
        <select onchange="repTech=this.value;renderRepairs()"><option value="">Technik (všetci)</option>${techs.map(t=>`<option value="${esc(t)}" ${repTech===t?"selected":""}>${esc(t)}</option>`).join("")}</select>
        <label class="chk" style="display:flex;align-items:center;gap:6px"><input type="checkbox" ${repWaiting?"checked":""} onchange="repWaiting=this.checked;renderRepairs()"> ⏱ Len čakajúce</label></div>
    </div>
    <div id="repReqs"></div>
    ${sections}`;
    restoreSearchFocus();loadRepairRequests();
  });
}
...
async function repairDetail(id){navHash("repairs/"+id);
  ...
  const {data:ships}=await sb.from("shipments").select("id,tracking_number,carrier,direction,status").eq("repair_id",id).order("id",{ascending:false});
  ...
  const shipsHtml=(ships&&ships.length)?ships.map(s=>`<div class="lot" style="display:flex;justify-content:space-between;align-items:center"><div onclick="shipDetail(${s.id})" style="cursor:pointer"><b class="mono">${esc(s.tracking_number||"—")}</b> <span class="muted">${esc(s.carrier||"")} · ${dirBadge(s.direction)}${s.status?" · "+esc(s.status):""}</span></div></div>`).join(""):`<div class="muted" style="font-size:12px">Zatiaľ bez zásielok.</div>`;
  const f=(k,v,cls)=>(v!==null&&v!==undefined&&v!=="")?`<div class="fld${cls?" "+cls:""}"><div class="k">${k}</div><div class="v">${v}</div></div>`:"";
  ...
  const na=!!r.not_repaired;
  const prog=REP_STAGES.map((s,i)=>{
    if(na&&REP_REPAIR_ONLY.has(s.k))return `<span class="tag" style="opacity:.5;text-decoration:line-through;margin:1px;font-size:11px" data-tip="${esc(s.label)} — neaplikovateľné (neopravuje sa)">${s.n}</span>`;
    return `<span class="tag ${i<doneIdx?"g":(i===doneIdx?(repStatusColor(s.k)||"b"):"")}" style="${i>doneIdx?"opacity:.4":""};margin:1px;font-size:11px" data-tip="${esc(s.label)}">${i<=doneIdx?"✓":""}${s.n}</span>`;}).join("");
  ...
  $("#view").innerHTML=SHIP2_CSS+`<div class="s2" style="max-width:none">
    <div class="card"><div class="chosen"><div><b>${esc(nm)}</b> <span class="mono" style="font-size:12px;color:var(--grey)">${esc(r.repair_no||"")}</span> ${r.kind==="reklamacia"?`<span class="tag r">reklamácia</span>`:`<span class="tag b">oprava</span>`} <span class="tag ${repStatusColor(r.status)}">${esc(repStageLabel(r.status))}</span></div>
      <div style="display:flex;gap:6px;flex-wrap:wrap">${canEd?`<button class="btn ghost sm" onclick="repairForm(${id})">✏️ Upraviť základ</button>`:""}<button class="btn ghost sm" onclick="repairReceipt(${id})">🧾 Preberací protokol</button><button class="btn ghost sm" onclick="repairProtocol(${id})">🧾 Protokol o oprave</button>${copyLinkBtn("repairs/"+id)}<button class="btn ghost sm" onclick="setTab('repairs')">Späť</button></div></div>
      ${na?`<div class="msg" style="background:var(--out-bg);color:var(--out-tx)">❌ <b>Neopravuje sa</b>${r.disposition?" · "+esc(r.disposition):""} — kroky ako potvrdenie ceny / diely / oprava sú neaplikovateľné.${canEd?` <button class="btn ghost sm" onclick="repairSetNotRepaired(${id},false)">✓ Predsa opraviť</button>`:""}</div>`:""}
      ...
      <div class="fgrid">
        ...
        ${f("Technik (rieši)",esc(r.technician||""))}${f("Naposledy zmenil",esc(r.last_by||""))}${f("Prijal",esc(r.received_by||""))}
      ...
      ${canEd?`<div class="inline" style="gap:6px;flex-wrap:wrap;margin-top:10px;align-items:center">
        ${na?`<span class="muted" style="font-size:12px">Ukončenie (neopravuje sa):</span><select onchange="repairSetDisposition(${id},this.value)"><option value="">— disposition —</option>${["vrátené neopravené","likvidácia","na diely"].map(d=>`<option ${r.disposition===d?"selected":""}>${d}</option>`).join("")}</select>`
        :`<button class="btn ghost sm" onclick="repairSetNotRepaired(${id},true)">❌ Neopravuje sa</button>`}</div>`:""}
    </div>
    <div class="card"><h4 style="margin:0 0 8px">🚚 Zásielky (doprava k oprave)</h4>${shipsHtml}
      ${canEd?`<div class="inline" style="gap:6px;flex-wrap:wrap;margin-top:8px"><button class="btn ghost sm" onclick="repairLinkShipment(${id},'inbound')">➕ Od zákazníka (prišlo)</button><button class="btn ghost sm" onclick="repairLinkShipment(${id},'outbound')">➕ Späť zákazníkovi (odoslané)</button></div>`:""}</div>
...
// neopravuje sa / disposition — kroky opravy sa stanú neaplikovateľné
async function repairSetNotRepaired(id,val){
  if(val){const reason=prompt("Dôvod (neopravuje sa) — napr. zákazník odmietol cenu / neplatí / neprebral:");if(reason===null)return;
    const {data:cur}=await sb.from("repairs").select("status").eq("id",id).single();
    await sb.from("repairs").update({not_repaired:true,updated_at:new Date().toISOString(),last_by:ME.email}).eq("id",id);
    await sb.from("repair_events").insert({repair_id:id,stage:(cur&&cur.status)||"diagnostika",note:"NEOPRAVUJE SA: "+(reason||""),data:{},created_by_name:ME.email});
  }else{await sb.from("repairs").update({not_repaired:false,disposition:null,updated_at:new Date().toISOString(),last_by:ME.email}).eq("id",id);
    await sb.from("repair_events").insert({repair_id:id,stage:"diagnostika",note:"Zrusene 'neopravuje sa' — bude sa opravovat",data:{},created_by_name:ME.email});}
  repairDetail(id);
}
async function repairProtocol(id){
  const {data:r}=await sb.from("repairs").select("*").eq("id",id).single();if(!r)return;
  const {data:evs}=await sb.from("repair_events").select("*").eq("repair_id",id).order("created_at",{ascending:true});
  const nm=r.title||((DATA.products.find(p=>p.id===r.product_id)||{}).name)||"(bez názvu)";
  const rowH=(l,v)=>v?`<tr><td style="padding:4px 14px 4px 0;color:#555;vertical-align:top"><b>${esc(l)}</b></td><td style="padding:4px 0">${esc(v)}</td></tr>`:"";
  const jour=(evs||[]).map(ev=>`<tr><td style="padding:3px 10px 3px 0;white-space:nowrap;color:#555">${esc(String(ev.created_at||"").slice(0,10))}</td><td>${esc(repStageLabel(ev.stage))}${ev.note?" — "+esc(ev.note):""}${ev.created_by_name?" ("+esc(ev.created_by_name)+")":""}</td></tr>`).join("");
  const html=`<!DOCTYPE html><html lang="sk"><head><meta charset="UTF-8"><title>Protokol o oprave ${esc(r.repair_no||"")}</title>
   <style>body{font-family:Arial,Helvetica,sans-serif;color:#222;max-width:800px;margin:24px auto;padding:0 16px}h1{border-bottom:3px solid #b9641b;padding-bottom:6px}table{border-collapse:collapse;width:100%}h2{font-size:15px;margin-top:20px}</style></head><body>
   <h1>Protokol o oprave</h1>
   <p><b>Číslo:</b> ${esc(r.repair_no||"")} &nbsp; <b>Dátum:</b> ${new Date().toLocaleDateString("sk-SK")}</p>
   <h2>Zákazník</h2><table>${rowH("Meno",r.customer)}${rowH("E-mail",r.customer_email)}${rowH("Telefón",r.customer_phone)}${rowH("Adresa",r.customer_address)}</table>
   <h2>Zariadenie</h2><table>${rowH("Názov",nm)}${rowH("Sériové číslo",r.serial)}${rowH("Popis závady",r.fault)}</table>
   <h2>Oprava</h2><table>${rowH("Odhad ceny",r.price_estimate!=null?r.price_estimate+" "+(r.price_currency||""):"")}${rowH("Finálna cena",r.price_final!=null?r.price_final+" "+(r.price_currency||""):"")}${rowH("Technik",r.technician)}${r.not_repaired?rowH("Neopravené",r.disposition||"áno"):""}</table>
   <h2>Priebeh</h2><table>${jour}</table>
   <p style="margin-top:36px">Podpis technika: __________________ &nbsp;&nbsp;&nbsp; Podpis zákazníka: __________________</p>
   <script>window.onload=function(){window.print()}<\/script></body></html>`;
  const w=window.open("","_blank");if(w){w.document.write(html);w.document.close();}
}
async function repairSetDisposition(id,val){const {data:cur}=await sb.from("repairs").select("status").eq("id",id).single();await sb.from("repairs").update({disposition:val||null,updated_at:new Date().toISOString(),last_by:ME.email}).eq("id",id);if(val)await sb.from("repair_events").insert({repair_id:id,stage:(cur&&cur.status)||"uzavrete",note:"Ukončenie (neopravené): "+val,data:{},created_by_name:ME.email});repairDetail(id);}
// prepojenie zásielky (doprava) na opravu — vytvorí zásielku s väzbou repair_id
async function repairLinkShipment(id,dir){
  const tn=prompt(dir==="inbound"?"Tracking číslo zásielky OD zákazníka (na opravu):":"Tracking číslo zásielky SPÄŤ zákazníkovi:");
  if(tn===null)return;const t=(tn||"").trim();if(!t)return;
  const carrier=detectCarrierName("",t)||"";
  const {error}=await sb.from("shipments").insert({tracking_number:t,carrier,direction:dir,repair_id:id});
  if(error){alert(error.message);return;}
  repairDetail(id);
}
...
async function repairAddEvent(id){
  ...
  const upd={status:repEventStage,updated_at:new Date().toISOString(),last_by:author};
  if(data.technician)upd.technician=data.technician;
  if(data.received_by)upd.received_by=data.received_by;
  if(data.expected)upd.parts_expected=data.expected;
...
let assetQ="",assetGroup="person",assetFPremise="",assetFPerson="",assetFRoom="",assetFCat="",assetPhoto="",assetPrefill=null,assetAssign="none";
...
function renderAssets(){
  $("#view").innerHTML=`<div class="card muted">Načítavam…</div>`;
  sb.from("assets").select("*").order("id",{ascending:false}).limit(2000).then(({data,error})=>{
    if(error){$("#view").innerHTML=`<div class="card"><div class="msg err">${esc(error.message)}</div></div>`;return;}
    let list=data||[];
    ...
    if(assetFCat){const ds=catDesc(Number(assetFCat));list=list.filter(a=>a.category_id&&ds.has(Number(a.category_id)));}
    ...
    const rowH=a=>`<tr onclick="assetDetail(${a.id})" style="cursor:pointer">
      <td><b>${esc(assetName(a))}</b>${a.category_id?`<div class="psub">${esc(catName(a.category_id))}</div>`:""}${a.serial?`<div class="psub">SN: ${esc(a.serial)}</div>`:""}${a.qr_code?`<div class="psub mono">${esc(a.qr_code)}</div>`:""}</td>
    ...
    const keyOf=a=>assetGroup==="person"?(a.person_id?("👤 "+personName(a.person_id)):"🏬 Sklad (nepriradené)")
      ...
      :assetGroup==="premise"?(premOf(a)?("🏢 "+premiseName(premOf(a))):"🏬 Sklad (nepriradené)")
      :assetGroup==="category"?(a.category_id?("🗂️ "+catName(a.category_id)):"— bez kategórie —"):"Všetok majetok";
    ...
    <div class="card">
      ...
      <div class="toolbar"><input id="assetQ" placeholder="Hľadať (názov / SN / QR / osoba / miestnosť)…" value="${esc(assetQ)}" oninput="assetQ=this.value;searchInput('assetQ',renderAssets)" style="flex:2 1 220px">
        <select onchange="assetGroup=this.value;renderAssets()">${gsel("person","Zoskupiť: osoba")}${gsel("room","Zoskupiť: miestnosť")}${gsel("premise","Zoskupiť: provozovňa")}${gsel("category","Zoskupiť: kategória")}${gsel("none","Bez zoskupenia")}</select>
        <select onchange="assetFCat=this.value;renderAssets()"><option value="">Kategória (všetky)</option>${catSorted().map(({c,depth})=>`<option value="${c.id}" ${assetFCat==String(c.id)?"selected":""}>${"   ".repeat(depth)}${depth?"↳ ":""}${esc(c.name)}</option>`).join("")}</select>
...
// ===== SPRÁVA EVIDENCIE: provozovne / miestnosti / osoby =====
function renderAssetAdmin(){
  const premRows=DATA.premises.map(pr=>`<div class="lot" style="display:flex;justify-content:space-between;align-items:center;gap:8px">
    <div><b>${esc(pr.name)}</b>${pr.address?`<div class="psub">${esc(pr.address)}</div>`:""}<div class="psub">${roomsOfPremise(pr.id).length} miestností</div></div>
    <div class="inline" style="gap:4px"><button class="btn ghost sm" onclick="apEditPremise(${pr.id})">✏️</button><button class="btn red sm" onclick="apDelPremise(${pr.id})">×</button></div></div>`).join("")||`<div class="muted">Žiadne provozovne.</div>`;
  const premOpts=DATA.premises.map(pr=>`<option value="${pr.id}">${esc(pr.name)}</option>`).join("");
  const roomRows=DATA.rooms.map(r=>`<div class="lot" style="display:flex;justify-content:space-between;align-items:center;gap:8px">
    <div><b>${esc(r.name)}</b> <span class="muted">· ${esc(premiseName(r.premise_id))}</span></div>
    <button class="btn red sm" onclick="apDelRoom(${r.id})">×</button></div>`).join("")||`<div class="muted">Žiadne miestnosti.</div>`;
  const persRows=DATA.persons.map(pe=>`<div class="lot" style="display:flex;justify-content:space-between;align-items:center;gap:8px">
    <div><b>${esc(pe.name)}</b>${pe.active===false?` <span class="tag">ukončený</span>`:""}${pe.email?`<div class="psub">${esc(pe.email)}</div>`:""}</div>
    <div class="inline" style="gap:4px"><button class="btn ghost sm" onclick="apEditPerson(${pe.id})">✏️</button>${pe.active!==false?`<button class="btn ghost sm" data-tip="Ukončiť osobu — jej majetok sa uvoľní (nepridelené)" onclick="apEndPerson(${pe.id})">⎋ Ukončiť</button>`:""}<button class="btn red sm" onclick="apDelPerson(${pe.id})">×</button></div></div>`).join("")||`<div class="muted">Žiadne osoby.</div>`;
  $("#view").innerHTML=`<div class="card"><div class="chosen"><b>Evidencia majetku</b><button class="btn ghost sm" onclick="setTab('assets')">← Späť na majetok</button></div></div>
  <div class="card"><h4>🏢 Provozovne</h4>${premRows}
    <div class="inline" style="gap:6px;margin-top:8px"><input id="ap_prem" placeholder="Nová provozovňa (názov)"><input id="ap_premaddr" placeholder="Adresa (nepovinné)"><button class="btn green sm" onclick="apAddPremise()">+ Pridať</button></div></div>
  <div class="card"><h4>📍 Miestnosti</h4>${roomRows}
    <div class="inline" style="gap:6px;margin-top:8px"><select id="ap_roomprem" style="flex:0 1 180px"><option value="">— provozovňa —</option>${premOpts}</select><input id="ap_room" placeholder="Názov miestnosti"><button class="btn green sm" onclick="apAddRoom()">+ Pridať</button></div></div>
  <div class="card"><h4>👤 Osoby</h4>${persRows}
    <div class="inline" style="gap:6px;margin-top:8px"><input id="ap_person" placeholder="Meno osoby"><input id="ap_personmail" placeholder="E-mail (nepovinné)"><button class="btn green sm" onclick="apAddPerson()">+ Pridať</button></div></div>`;
}
async function apAddPremise(){const n=($("#ap_prem").value||"").trim();if(!n)return;const addr=($("#ap_premaddr").value||"").trim()||null;const {error}=await sb.from("asset_premises").insert({name:n,address:addr,sort_order:(DATA.premises.length+1)});if(error){alert(error.message);return;}await loadData();renderAssetAdmin();}
async function apEditPremise(id){const pr=DATA.premises.find(x=>x.id===id);if(!pr)return;const n=prompt("Názov provozovne:",pr.name);if(n===null)return;const addr=prompt("Adresa:",pr.address||"");const {error}=await sb.from("asset_premises").update({name:n.trim()||pr.name,address:(addr||"").trim()||null}).eq("id",id);if(error){alert(error.message);return;}await loadData();renderAssetAdmin();}
async function apDelPremise(id){if(!confirm("Zmazať provozovňu aj jej miestnosti?"))return;const {error}=await sb.from("asset_premises").delete().eq("id",id);if(error){alert(error.message);return;}await loadData();renderAssetAdmin();}
async function apAddRoom(){const prem=$("#ap_roomprem").value;const n=($("#ap_room").value||"").trim();if(!prem||!n){alert("Vyber provozovňu a napíš názov miestnosti.");return;}const {error}=await sb.from("asset_rooms").insert({premise_id:Number(prem),name:n,sort_order:(DATA.rooms.length+1)});if(error){alert(error.message);return;}await loadData();renderAssetAdmin();}
async function apDelRoom(id){if(!confirm("Zmazať miestnosť?"))return;const {error}=await sb.from("asset_rooms").delete().eq("id",id);if(error){alert(error.message);return;}await loadData();renderAssetAdmin();}
async function apAddPerson(){const n=($("#ap_person").value||"").trim();if(!n)return;const mail=($("#ap_personmail").value||"").trim()||null;const {error}=await sb.from("asset_persons").insert({name:n,email:mail,active:true,sort_order:(DATA.persons.length+1)});if(error){alert(error.message);return;}await loadData();renderAssetAdmin();}
async function apEditPerson(id){const pe=DATA.persons.find(x=>x.id===id);if(!pe)return;const n=prompt("Meno osoby:",pe.name);if(n===null)return;const mail=prompt("E-mail:",pe.email||"");const {error}=await sb.from("asset_persons").update({name:n.trim()||pe.name,email:(mail||"").trim()||null}).eq("id",id);if(error){alert(error.message);return;}await loadData();renderAssetAdmin();}
async function apDelPerson(id){const {count}=await sb.from("assets").select("id",{count:"exact",head:true}).eq("person_id",id);if(count){alert("Osoba má pridelený majetok ("+count+"). Najprv ju ukonči (⎋) alebo majetok prepriraď.");return;}if(!confirm("Zmazať osobu?"))return;const {error}=await sb.from("asset_persons").delete().eq("id",id);if(error){alert(error.message);return;}await loadData();renderAssetAdmin();}
async function apEndPerson(id){const pe=DATA.persons.find(x=>x.id===id);if(!pe)return;
  const {data:mine}=await sb.from("assets").select("id").eq("person_id",id);
  const n=(mine||[]).length;
  if(!confirm("Ukončiť osobu „"+pe.name+"“? Jej "+n+" položiek majetku sa uvoľní (nepridelené, k dispozícii na preradenie)."))return;
  for(const a of (mine||[])){await sb.from("assets").update({person_id:null}).eq("id",a.id);await assetLogMove(a.id,"unassign",{},"ukončenie osoby: "+pe.name);}
  await sb.from("asset_persons").update({active:false}).eq("id",id);
  await loadData();renderAssetAdmin();
}
// ===== FORMULÁR MAJETKU =====
function assetForm(id){
  assetPhoto="";
  const pre=assetPrefill||{};assetPrefill=null;
  const load=id?sb.from("assets").select("*").eq("id",id).single():Promise.resolve({data:{state:"used",...pre}});
  load.then(({data:a})=>{
    assetPhoto=a.image_url||"";
    assetAssign=a.person_id?"person":(a.room_id?"room":"none");
    const prodOpts=DATA.products.map(p=>`<option value="${esc(p.name)}"></option>`).join("");
    ...
    const personOpts=DATA.persons.filter(pe=>pe.active!==false||pe.id===a.person_id).map(pe=>`<option value="${pe.id}" ${a.person_id===pe.id?"selected":""}>${esc(pe.name)}</option>`).join("");
    const premOpts=DATA.premises.map(pr=>`<option value="${pr.id}" ${a.premise_id===pr.id?"selected":""}>${esc(pr.name)}</option>`).join("");
...
async function assetSave(id){
  ...
  // QR kód: ak prázdny, prideľ automaticky "AST-<id>"
  if(newId&&!o.qr_code){await sb.from("assets").update({qr_code:"AST-"+newId}).eq("id",newId);}
  // pohyb majetku
  await assetLogMove(newId,id?"edit":"in",{person_id,room_id,premise_id},id?"úprava":"pridané do majetku");
  // ak vzniká zo skladu — vyskladni pôvodnú šaržu
  if(!id&&srcLot){try{
    const {data:lot}=await sb.from("stock_lots").select("*").eq("id",srcLot).single();
    if(lot){const dn=await nextDocNo("vydaj");await sb.from("stock_movements").insert({type:"vydaj",product_id:lot.product_id,lot_id:lot.id,quantity:1,warehouse_id:lot.warehouse_id,location_id:lot.location_id,purpose:"presun do majetku",note:"→ Majetok",doc_no:dn});
...
// presun zo skladu → predvyplní formulár majetku
async function assetPickFromStock(){
  ...
  $("#view").innerHTML=`<div class="card"><div class="chosen"><b>Presun zo skladu do majetku</b><button class="btn ghost sm" onclick="setTab('assets')">Späť</button></div>
    <div class="muted" style="margin-bottom:8px">Vyber skladovú položku — vytvorí sa karta majetku a kus sa vyskladní (výdajka).</div>
...
async function assetDetail(id){navHash("assets/"+id);
  ...
  if(error){alert(error.message);return;}
  const {data:mov}=await sb.from("asset_movements").select("*").eq("asset_id",id).order("happened_at",{ascending:false}).limit(30);
  const row=(l,v)=>(v!==null&&v!==undefined&&v!=="")?`<div class="lot"><div class="m">${esc(l)}</div><b>${v}</b></div>`:"";
  const st=ASSET_STATE[a.state]||[a.state,""];
  const ACT={"in":"pridané do majetku","edit":"úprava","assign":"pridelené","unassign":"uvoľnené","move":"presun","tostock":"vrátené na sklad"};
  const movHtml=(mov&&mov.length)?mov.map(m=>`<div class="lot"><div class="m">${esc(String(m.happened_at||"").replace("T"," ").slice(0,16))}${m.by_user?" · "+esc(m.by_user):""}</div><b>${esc(ACT[m.action]||m.action||"")}</b>${(m.person_id||m.room_id)?` <span class="muted">→ ${m.person_id?esc(personName(m.person_id)):esc(roomName(m.room_id))}</span>`:""}${m.note?`<div class="psub">${esc(m.note)}</div>`:""}</div>`).join(""):`<div class="muted">Bez pohybov.</div>`;
  $("#view").innerHTML=`<div class="card"><div class="chosen"><div><b>${esc(assetName(a))}</b> <span class="tag ${st[1]}">${esc(st[0])}</span></div>
    <div style="display:flex;gap:6px;flex-wrap:wrap">${canWrite()?`<button class="btn ghost sm" onclick="assetForm(${id})">✏️ Upraviť</button><button class="btn ghost sm" onclick="assetReassign(${id})">🔁 Prideliť / presunúť</button><button class="btn ghost sm" onclick="assetToStock(${id})">↩︎ Vrátiť na sklad</button>`:""}${copyLinkBtn("assets/"+id)}<button class="btn ghost sm" onclick="setTab('assets')">Späť</button></div></div>
    ${a.image_url?`<img src="${esc(a.image_url)}" style="max-width:200px;border-radius:10px;border:1px solid var(--line);margin:6px 0">`:""}
    ${row("Priradené",assetAssignHtml(a))}
    ${row("Kategória",a.category_id?esc(catPathText(a.category_id)):"")}
    ${row("QR kód",a.qr_code?esc(a.qr_code)+copyBtn(a.qr_code):"")}
    ${row("Sériové číslo",a.serial?esc(a.serial)+copyBtn(a.serial):"")}
    ${row("Nadobudnuté",a.acquired_at?String(a.acquired_at).slice(0,10):"")}${row("Poznámka",esc(a.note||""))}
...
async function assetExport(){
  ...
  const rows=(data||[]).map(a=>[assetName(a),catPathText(a.category_id)||"",a.serial||"",a.qr_code||"",personName(a.person_id)||a.holder||"",roomName(a.room_id)||a.room||"",premiseName(a.premise_id)||"",(ASSET_STATE[a.state]||[a.state])[0],a.acquired_at||"",a.note||""]);
  const head=["Položka","Kategória","Sériové číslo","QR","Osoba","Miestnosť","Provozovňa","Stav","Nadobudnuté","Poznámka"];
